...
### 覆核等級
- `正常`：沒有觸發距離、候選、移動時間或住家軌跡風險。
- `低信心`：有輕微不確定性，例如附近候選院所密集，但未達需人工覆核。
- `需覆核`：需要主管或助理回看打卡點脈絡，例如系統選定既有客戶距離偏遠、候選排名過後、住家起訖但缺少外勤佐證。
- `高風險需覆核`：有明顯不合理訊號，例如相鄰行程移動時間不合理、單日風險分數偏高、員工期間風險率偏高，或出現僅住家附近軌跡。

### 事件層級原因代碼
- `far_customer_override`：系統選到較遠既有客戶，但附近存在更近候選院所。
- `selected_not_top5`：系統選定院所不在前五近候選內。
- `selected_distance_too_far`：系統選定院所距離超過自動選取門檻。
- `nearby_candidate_conflict`：附近候選院所過於密集，單點判定信心較低。
- `no_reasonable_candidate`：GPS 點附近沒有合理候選院所。
- `impossible_travel_time`：相鄰 GPS 點或路線段所需移動時間不合理。

### 住家軌跡規則
- `home_area_only_trace`：當日 GPS 主要停留在住家附近，且沒有足夠外勤拜訪佐證。這種情況會提高日風險，因為只在家附近上下班打卡無法證明實際外勤軌跡。
- `home_start_end_without_field_trace`：上下班點接近住家，但中間沒有足夠遠離住家的外勤拜訪或合理候選院所佐證。
- `insufficient_route_evidence`：GPS 點數、路線或候選匹配不足，無法形成可檢查的外勤路徑。
- 合理外勤佐證包含系統選定院所，或距離合理的第一候選院所；但若該點仍在住家半徑內，不會被視為外勤佐證。

### 員工排序
- 日層級風險會彙總為 `risk_score`、`review_event_count`、`high_risk_event_count` 與 `risk_rate`。
- 員工總覽使用 `risk_score / GPS點數` 作為期間風險率，避免少量 GPS 的單日資料被過度放大或高 GPS 天數被低估。
- 風險排序只用來決定覆核優先順序，不直接取代人工核定。
